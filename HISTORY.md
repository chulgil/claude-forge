...
### 하네스 엔지니어링 패턴 도입 (자체 추가)

엔터프라이즈 AX 현장의 하네스 엔지니어링(스페이스YY) 패턴 2가지를 통합.

| 변경 | 파일 | 사용법 |
|------|------|--------|
| **CPS 프레임** | `commands/cps.md` (신규) | `/cps 요구사항 설명` |
| **도메인 린터** | `rules/domain-linter.md` (신규) | 매 세션 자동 로딩 |

- `/cps`: 고객 요구사항을 Context·Problem·Solution으로 구조화. `/plan` 앞 단계.
- 도메인 린터: 파일명·클래스명·임포트 순서·폴더 구조를 강제하여 AI 모델 무관 멱등성 보장.
